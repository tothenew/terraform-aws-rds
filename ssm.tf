...
resource "aws_ssm_parameter" "rds_reader_endpoint" {
  ...
  name        = var.project == "" ? "/${var.environment}/${local.identifier}/${local.current_day}-${local.current_time}/rds/reader_endpoint" : "/${var.environment}/${var.project}/${local.identifier}/${local.current_day}-${local.current_time}/rds/reader_endpoint"
...
resource "aws_ssm_parameter" "rds_db_address" {
  count       = var.secret_method == "ssm" ? 1 : 0
  name        = var.project == "" ? "/${var.environment}/${local.identifier}/${local.current_day}-${local.current_time}/rds/host" : "/${var.environment}/${var.project}/${local.identifier}/${local.current_day}-${local.current_time}/rds/host"
